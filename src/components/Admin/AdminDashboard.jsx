...
const AdminDashboard = () => {
  ...
  const statsCards = [
    {
      title: t("dashboard.total_stops"),
      value: loading ? '...' : dashboardData.totalBusStops.toString(),
      unit: t("dashboard.stops_unit"),
      icon: MapPin,
      color: 'bg-blue-500',
      textColor: 'text-blue-600'
    },
    {
      title: t("dashboard.total_routes"),
      value: loading ? '...' : dashboardData.totalRoutes.toString(),
      unit: t("dashboard.routes_unit"),
      icon: Route,
      color: 'bg-green-500',
    ...
    },
    {
      ...
      unit: t("dashboard.students_unit"),
      icon: Users,
      color: 'bg-purple-500',
      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            {t("dashboard.total_students_label")}: <span className="font-bold text-blue-600">{payload[0].value}</span>
          </p>
        </div>
      );
    }
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-300 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">
            {t("dashboard.account_count")}: <span className="font-bold">{payload[0].value}</span>
          </p>
          <p className="text-sm text-gray-600">
            {t("dashboard.percentage")}: <span className="font-bold">{percentage}%</span>
          </p>
        </div>
      );
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-800">
                📈 {t("dashboard.student_usage_chart")}
              </h2>
              <select
                value={selectedYear}
              ...
              >
                {availableYears.map(year => (
                  <option key={year} value={year}>{t("dashboard.year")} {year}</option>
                ))}
              </select>
            </div>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={studentTrendData}>
                ...
                <Line
                  ...
                  stroke="#3b82f6"
                  strokeWidth={3}
                  name={t("dashboard.total_students_label")}
                  dot={{ fill: '#3b82f6', r: 4 }}
                  activeDot={{ r: 6 }}
                />
              </LineChart>
          ...
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-800">
                🚌 {t("dashboard.top_routes_chart")}
              </h2>
              <button
                onClick={() => setShowRouteTable(true)}
                className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              >
                {t("dashboard.view_all_routes")}
              </button>
            </div>
            <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-md p-6">
            <h2 className="text-xl font-bold text-gray-800 mb-4">
              👥 {t("dashboard.user_classification")}
            </h2>
            {loading ? (
              <div className="flex items-center justify-center h-[300px]">
                <div className="text-gray-500">{t("dashboard.loading_data")}</div>
              </div>
              ...
              <div className="flex items-center justify-center h-[300px]">
                <div className="text-gray-500">{t("dashboard.no_data")}</div>
              </div>
            )}
          </div>
        </div>
                ...
                <thead className="bg-gray-50 sticky top-0">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t("dashboard.route_code")}
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t("dashboard.route_name")}
                    </th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t("dashboard.student_count")}
                    </th>
                  </tr>
                </thead>
