...
import { getDriverLocationsForParent } from "../../services/userService";
...
const Map_PhuHuynh = () => {
  ...
  const [driverLocations, setDriverLocations] = useState([]); // THÊM STATE NÀY
  ...
  // THÊM: Icon cho driver
  const driverIcon = new L.Icon({
    iconUrl: "/logo.png", // Mày cần có file này trong public folder
    iconSize: [50, 50],
    iconAnchor: [16, 32],
    popupAnchor: [0, -32],
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(t("parent_map.map_init_error"), error);
    }
  ...
  // THÊM: Fetch driver locations real-time
  useEffect(() => {
    const fetchDriverLocations = async () => {
      try {
        ...
        const response = await getDriverLocationsForParent(userInfo.id_user);

        if (response.data.errCode === 0) {
          setDriverLocations(response.data.data);
          renderDriverMarkers(response.data.data);
        }
      } catch (error) {
        console.error(t("parent_map.fetch_driver_locations_error"), error);
      }
    };
    ...
    if (mapReady) {
      ...
      // Set up interval để update real-time (mỗi 10 giây)
  ...
  // THÊM: Function để render driver markers
  const renderDriverMarkers = (drivers) => {
    // Clear existing driver markers
    markersRef.current = markersRef.current.filter(marker => {
      if (marker.driverMarker && mapInstanceRef.current) {
    ...
    drivers.forEach(driver => {
      if (driver.toado_x && driver.toado_y && !isNaN(driver.toado_x) && !isNaN(driver.toado_y)) {
        const driverMarker = L.marker(
          [driver.toado_x, driver.toado_y],
          { icon: driverIcon }
        ).addTo(mapInstanceRef.current);

        driverMarker.bindPopup(`
          <div style="text-align: center; min-width: ${isMobile ? '150px' : '200px'};">
            <b style="font-size: ${isMobile ? '12px' : '14px'};">🚗 ${t("parent_map.driver")}: ${driver.driver_name}</b><br>
            <small style="font-size: ${isMobile ? '10px' : '12px'};">${t("parent_map.driver_code")}: ${driver.id_driver}</small><br>
            <small style="font-size: ${isMobile ? '10px' : '12px'};">${t("parent_map.schedule")}: ${driver.schedule_time}</small><br>
            <small style="font-size: ${isMobile ? '9px' : '11px'}; color: #666;">
              ${t("parent_map.updated")}: ${new Date(driver.last_updated).toLocaleTimeString()}
            </small>
          </div>
        `);

        driverMarker.driverMarker = true;
  ...
  const fetchStudentsData = async () => {
    try {
      ...
      if (!userInfo) {
        console.error(t("parent_map.not_logged_in"));
        return;
      }
      ...
      console.error(t("parent_map.load_students_error"), error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const addResetViewControl = (map) => {
    ...
    resetControl.onAdd = function () {
      ...
      div.innerHTML = `
                ...
                ">🏫 ${t("parent_map.back_to_school")}</button>
            `;
  ...
  const clearMap = () => {
    // Clear markers (chỉ clear bus stop markers, giữ driver markers)
    markersRef.current.forEach(marker => {
      if (mapInstanceRef.current && !marker.driverMarker) {
        mapInstanceRef.current.removeLayer(marker);
      }
    });

    // Chỉ giữ lại driver markers
    markersRef.current = markersRef.current.filter(marker => marker.driverMarker);
  ...
  const updateMapForSelectedStudents = async (studentIds) => {
    ...
    // Fit map to show all routes (bao gồm cả driver markers)
  ...
  const drawStudentRoute = async (student) => {
    try {
      ...
      if (!student.busstop || !student.busstop.id_busstop) {
        console.warn(t("parent_map.student_no_bus_stop", { name: student.name }));
        return;
      }
      ...
      if (typeof busStop.toado_x !== 'number' || typeof busStop.toado_y !== 'number' ||
        ...
        console.warn(t("parent_map.invalid_coordinates", { name: student.name }), {
      ...
      busStopMarker.bindPopup(`
                <div style="text-align: center; min-width: ${isMobile ? '150px' : '200px'};">
                    <b style="font-size: ${isMobile ? '12px' : '14px'};">
                        🚌 ${busStop.name_station || t("parent_map.bus_stop")}
                    </b><br>
                    <small style="color: #666; font-size: ${isMobile ? '10px' : '12px'};">
                        ${t("parent_map.student")}: ${student.name}
                    </small><br>
                    <small style="color: #666; font-size: ${isMobile ? '10px' : '12px'};">
                        ${t("parent_map.class")}: ${student.class}
                    </small><br>
      ...
      try {
        ...
        if (routeResponse.data.errCode === 0 && routeResponse.data.data &&
          ...
          if (validBusStops.length >= 2) {
            ...
            console.warn(t("parent_map.not_enough_valid_stops", { name: student.name }));
          }
        } else {
          console.log(t("parent_map.no_route_found", { busStopId: busStop.id_busstop }));
        }
      } catch (routeError) {
        console.warn(t("parent_map.cannot_get_route", { name: student.name }), routeError);
        // Vẫn hiển thị marker bus stop dù không có route
      }

    } catch (error) {
      console.error(t("parent_map.draw_route_error", { name: student.name }), error);
    }
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h3 style={{ margin: 0, color: "#333" }}>
            👨‍👩‍👧‍👦 {t("parent_map.students")} ({students.length})
          </h3>
          <button
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{
                ...
                🚗 {t("parent_map.active_drivers")} ({driverLocations.length})
              </span>
            </div>
            {driverLocations.map((driver, index) => (
              <div key={driver.id_driver} style={{
                fontSize: "12px",
                color: "#475569",
                marginTop: index > 0 ? "4px" : "0"
              }}>
                • {driver.driver_name} - {driver.schedule_time}
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ maxHeight: "calc(100% - 100px)", overflowY: "auto" }}>
          {loading ? (
            <div style={{ textAlign: "center", padding: "20px", color: "#666" }}>
              🔄 {t("parent_map.loading")}
            </div>
          ) : students.length === 0 ? (
            <div style={{ textAlign: "center", padding: "20px", color: "#666" }}>
              📝 {t("parent_map.no_students")}
            </div>
          ) : (
      ...
      <div style={{
        ...
        {!mapReady && (
          <div style={{
            ...
            <div>🔄 {t("parent_map.initializing_map")}</div>
          </div>
        )}
...
// import { getStudentsByParent } from "../../services/studentService";
// import { getBusStopsByRoute } from "../../services/routeService";
// import { getDriverLocationsForParent } from "../../services/userService"; // THÊM DÒNG NÀY
// import { drawSchoolBoundary } from "../SchoolBoundary";
...
//         driverMarker.bindPopup(`
//           <div style="text-align: center; min-width: ${isMobile ? '150px' : '200px'};">
//             <b style="font-size: ${isMobile ? '12px' : '14px'};">🚗 Tài xế: ${driver.driver_name}</b><br>
//             <small style="font-size: ${isMobile ? '10px' : '12px'};">Mã TX: ${driver.id_driver}</small><br>
//             <small style="font-size: ${isMobile ? '10px' : '12px'};">Lịch: ${driver.schedule_time}</small><br>
//             <small style="font-size: ${isMobile ? '9px' : '11px'}; color: #666;">
//               Cập nhật: ${new Date(driver.last_updated).toLocaleTimeString()}
//             </small>
//           </div>
//         `);
...
//                 color: "#0c4a6e"
//               }}>
//                 🚗 Tài xế đang hoạt động ({driverLocations.length})
//               </span>
//             </div>
//             {driverLocations.map((driver, index) => (
//               <div key={driver.id_driver} style={{
